    ...
    public set reviewData(data: PlacementReviewDataModel) {
        if (data) {
            ...
            if (this.allReviewData.assignment) {
                if (this.allReviewData?.person?.birthDate) this.allReviewData.person.birthDate = this.dateFormatService.convert_mmddyyyy_hypen_to_mmddyyyy_slash(this.allReviewData.person.birthDate);
            }
        }
    }
